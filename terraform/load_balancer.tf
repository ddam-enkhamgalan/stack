...
resource "aws_lb_listener_rule" "api" {
  ...
  condition {
    path_pattern {
      values = ["/api/*", "/health", "/openapi.json", "/api-docs/*"]
    }
  }
}

# If no certificate, use HTTP listener for development
resource "aws_lb_listener" "web_http_dev" {
  count             = var.certificate_arn == "" ? 1 : 0
  load_balancer_arn = aws_lb.main.arn
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_lb_listener_rule" "api_http_dev" {
  count        = var.certificate_arn == "" ? 1 : 0
  listener_arn = aws_lb_listener.web_http_dev[0].arn
  priority     = 100
